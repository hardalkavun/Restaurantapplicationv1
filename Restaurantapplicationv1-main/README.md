...
- Roles: `MANAGER`, `COURIER`, and `CUSTOMER`
- Customer menu preview and order placement
- Manager access to dashboard, categories, products, order approval/cancellation, and courier assignment
- Courier access to assigned order list/detail and delivery status updates
- Category CRUD
- Product CRUD with image upload to MySQL `LONGBLOB`
- Product image serving from database bytes through `/products/{id}/image`
- Order detail, customer order placement, courier assignment, approval/cancellation, and status update
...
6. Log in with `manager / 1234`, `courier / 1234`, or `customer / 1234`.

## Order Workflow

Customers place orders from `/customer/menu`. New orders start as `PENDING`.

Managers use `/orders` to review incoming orders, assign a courier, approve orders by moving them to `PREPARING`, or cancel orders.

Couriers use `/orders` to see only their assigned deliveries. They can update assigned orders to `ON_THE_WAY` or `DELIVERED`.
